...
=======
  nfatodfa_epsilon(): any {
    ...
    if (dfa_path.includes('E')) {
      dfa_path.splice(dfa_path.indexOf('E'), 1);
>>>>>>> c514d471e11f46a4350ecfa35960fd8b68bff825
    }
...
<<<<<<< HEAD
    ...
    while (dfa_state_test.length > 0){
      ...
      dfa_path.forEach(function(j,jindex){
        ...
        if (check_end_state && !dfa_end_state.includes(tmp_state)) dfa_end_state.push(tmp_state)
        dfa_transition[i][j] = tmp_state

        if (dfa_state_test.includes(tmp_state)==false) dfa_state_test.push(tmp_state)
      })
      dfa_state.push(i)
