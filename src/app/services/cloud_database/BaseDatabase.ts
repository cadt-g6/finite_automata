...
abstract class BaseDatabase<T> {
  ...
  async fetchAll(
    ...
    if (items.length > 0) {
      nextPageKey = result.docs[result.docs.length - 1].id;
    }
  ...
  collectionReference(): db.CollectionReference<T> {
    const reference = db.collection(
      BaseDatabase.instance,
      this.collectionName(),
    );
    return reference.withConverter<T>(this.objectConverter());
  }
  ...
  constructQueryOptions(
    ...
    // if (nextPageKey) {
    //   queryOptions.push(db.startAfter(5));
    // }
